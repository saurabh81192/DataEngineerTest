...
def test_duplicate_order_ids():
    ...
    duplicate_count = orders_df.groupBy(
        "`Order ID`"
    ).count().filter(
        col("count") > 1
    ).count()
...
def test_customer_name_special_chars():
    ...
    invalid = master_df.filter(
        col("customer_name").rlike(r"[^a-zA-Z\s]")
    ).count()
...
def test_product_id_format():
    ...
    invalid = master_df.filter(
        col("product_id").rlike(r"^P\d+$") == False
    ).count()
...
def test_profit_reconciliation():
    ...
    assert round(master_profit, 2) == round(aggregate_profit, 2)
